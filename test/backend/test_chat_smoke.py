"""AI 聊天接口冒烟：鉴权、会话 CRUD、API Key 状态（不强制真实 DeepSeek）。"""
...
def test_admin_chat_menu_and_crud(client):
    ...
    created = client.post(
        ...
        headers=headers,
        json={"title": "smoke_chat", "model": "deepseek-chat"},
    )
    ...
    body = stream.text
    assert "API Key" in body or "error" in body
...
def test_portal_chat_crud(client):
    ...
    created = client.post(
        ...
        headers=headers,
        json={"title": "portal_smoke", "model": "deepseek-chat"},
    )
    assert created.json()["code"] == "00000", created.text
    conv_id = created.json()["data"]["id"]
